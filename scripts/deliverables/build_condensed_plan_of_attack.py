"""
Build a condensed combined Plan of Attack PDF.

Changes from the original Section 1:
  - Removed the "Outcome and treatment construction" subsection
  - Removed the "Contingencies" subsection

Sections 2 and 3 are unchanged (pulled from existing PDFs).

Output: deliverables/plan_of_attack_combined_condensed.pdf
...
def build_section_1_bytes():
    """Build condensed Section 1 and return as bytes."""
    buf = io.BytesIO()
    ...
    story.append(Paragraph("Costco Australia / Synthetic Control", subtitle_style))
    ...
    story.append(Paragraph(
        ...
        "565 unique postcodes, and 11 unique fuel-type codes (regular "
        "unleaded ULP/U91/E10, premium PUL95/PUL98, diesel DSL/PDL, LPG, "
        "and others). Coordinates are not in the historical files; we "
        "attached lat/lng by joining station name + postcode to a "
        "one-time pull from the live FuelCheck API.",
        body_style,
    ))
    nsw_table = _table([
        [Paragraph("<b>Field</b>", cell_style),
         Paragraph("<b>Description</b>", cell_style)],
        [Paragraph("ServiceStationName", cell_style),
        ...
        [Paragraph("Suburb", cell_style),
         Paragraph("Suburb", cell_style)],
        [Paragraph("Postcode", cell_style),
         Paragraph("Postcode (4-digit)", cell_style)],
        [Paragraph("Brand", cell_style),
         Paragraph("Brand affiliation (Costco, Caltex, Shell, BP, etc.)", cell_style)],
        [Paragraph("FuelCode", cell_style),
         Paragraph("Fuel-type code (U91, E10, PUL95, PUL98, DSL, LPG, ...)", cell_style)],
        [Paragraph("PriceUpdatedDate", cell_style),
    ...
    qld_table = _table([
        [Paragraph("<b>Field</b>", cell_style),
         Paragraph("<b>Description</b>", cell_style)],
        [Paragraph("SiteId", cell_style),
         Paragraph("Stable per-station numeric identifier", cell_style)],
        [Paragraph("Site_Name", cell_style),
         Paragraph("Station name", cell_style)],
        [Paragraph("Site_Brand", cell_style),
         Paragraph("Brand affiliation", cell_style)],
        [Paragraph("Sites_Address_Line_1", cell_style),
         Paragraph("Street address", cell_style)],
        [Paragraph("Site_Suburb", cell_style),
         Paragraph("Suburb", cell_style)],
        [Paragraph("Site_Post_Code", cell_style),
         Paragraph("Postcode (4-digit)", cell_style)],
        [Paragraph("Site_Latitude / Site_Longitude", cell_style),
         Paragraph("WGS84 coordinates (used for great-circle distance to Costco)", cell_style)],
        [Paragraph("Fuel_Type", cell_style),
         Paragraph("Fuel-type label (Unleaded 91, e10, Premium 95, ...)", cell_style)],
        [Paragraph("Price", cell_style),
         Paragraph("Posted retail price in tenths of a cent (1899 = 189.9 \u00a2/L)", cell_style)],
        [Paragraph("TransactionDateutc", cell_style),
    ...
    story.append(Paragraph(
        ...
        "NSW and QLD, FuelWatch publishes one record per station per day "
        "regardless of whether the price changed. Composite unique key: "
        "<font face='Courier'>(TRADING_NAME, ADDRESS, PRODUCT_DESCRIPTION, "
        "PUBLISH_DATE)</font>. The file contains 1,423 unique stations, "
        "47 brands, 200 postcodes, and 7 fuel-type strings. Coordinates "
        "are not in the historical files; we attached lat/lng by joining "
        "station name + postcode to a one-time pull from the live "
        "FuelWatch API.",
        body_style,
    ))
    wa_table = _table([
        [Paragraph("<b>Field</b>", cell_style),
         Paragraph("<b>Description</b>", cell_style)],
        [Paragraph("PUBLISH_DATE", cell_style),
         Paragraph("Calendar day of the snapshot", cell_style)],
        [Paragraph("TRADING_NAME", cell_style),
         Paragraph("Station name", cell_style)],
        [Paragraph("BRAND_DESCRIPTION", cell_style),
         Paragraph("Brand affiliation", cell_style)],
        [Paragraph("PRODUCT_DESCRIPTION", cell_style),
         Paragraph("Fuel-type label (ULP, 98 RON, Diesel, LPG, ...)", cell_style)],
        [Paragraph("PRODUCT_PRICE", cell_style),
         Paragraph("Posted retail price (cents per liter)", cell_style)],
        [Paragraph("ADDRESS", cell_style),
         Paragraph("Street address", cell_style)],
        ...
        [Paragraph("POSTCODE", cell_style),
         Paragraph("Postcode (4-digit)", cell_style)],
        [Paragraph("AREA_DESCRIPTION / REGION_DESCRIPTION", cell_style),
         Paragraph("Government-defined regional grouping (used as a secondary geographic check)", cell_style)],
    ...
    # ---- Treated/excluded Costcos table ----
    story.append(Paragraph(
    ...
    treated_table = _table([
        ...
        [Paragraph("Coomera", cell_style), Paragraph("QLD", cell_style),
         Paragraph("May 2023", cell_style), Paragraph("51", cell_style),
         Paragraph("31", cell_style), Paragraph("Treated", cell_style)],
        [Paragraph("Casuarina", cell_style), Paragraph("WA", cell_style),
         Paragraph("Nov 2022", cell_style), Paragraph("58", cell_style),
         Paragraph("42", cell_style), Paragraph("Treated", cell_style)],
        [Paragraph("Perth Airport", cell_style), Paragraph("WA", cell_style),
         Paragraph("Feb 2020", cell_style), Paragraph("26", cell_style),
         Paragraph("74", cell_style), Paragraph("Treated", cell_style)],
        [Paragraph("Lake Macquarie", cell_style), Paragraph("NSW", cell_style),
         Paragraph("May 2022", cell_style), Paragraph("58", cell_style),
         Paragraph("35", cell_style), Paragraph("Treated", cell_style)],
        [Paragraph("Marsden Park", cell_style), Paragraph("NSW", cell_style),
         Paragraph("Jul 2017", cell_style), Paragraph("11", cell_style),
         Paragraph("105", cell_style),
         Paragraph("Excluded (pre too short)", cell_style)],
        [Paragraph("Auburn", cell_style), Paragraph("NSW", cell_style),
         Paragraph("Aug 2025", cell_style), Paragraph("107", cell_style),
         Paragraph("8", cell_style),
         Paragraph("Excluded (post too short)", cell_style)],
        [Paragraph("Canberra Airport", cell_style), Paragraph("ACT", cell_style),
         Paragraph("Nov 2023", cell_style), Paragraph("0", cell_style),
         Paragraph("29", cell_style),
         Paragraph("Excluded (no pre)", cell_style)],
        [Paragraph("Casula", cell_style), Paragraph("NSW", cell_style),
         Paragraph("Dec 2016", cell_style), Paragraph("4", cell_style),
         Paragraph("113", cell_style),
         Paragraph("Excluded (pre-registry)", cell_style)],
        [Paragraph("North Lakes", cell_style), Paragraph("QLD", cell_style),
         Paragraph("Jan 2019", cell_style), Paragraph("1", cell_style),
         Paragraph("89", cell_style),
         Paragraph("Excluded (pre-registry)", cell_style)],
        [Paragraph("Ipswich", cell_style), Paragraph("QLD", cell_style),
         Paragraph("Mar 2019", cell_style), Paragraph("3", cell_style),
    ...
    # ---- Figure 4: treated event studies ----
    ...
    story.append(Paragraph(
        ...
        "¢/L). After our 50&ndash;500 ¢/L sanity filter, the "
        "post-aggregation panel of 17,953 postcode-months has no "
        "observations outside 92&ndash;259 ¢/L, well within plausible "
        "Australian range.",
        body_style,
    ))
    story.append(Paragraph(
        "<b>Treated-Costco coverage variance.</b> Coomera&rsquo;s "
        "pre-2020 QLD coverage is sparse because the QLD scheme began in "
        "December 2018 and Coomera-area stations entered the registry "
        "gradually; the relevant pre-period (Sep 2020 \u2192 May 2023) is "
        "fully covered. Casuarina has only ~3 unique stations within 5 km "
        "contributing to the treated mean, reflecting a sparse coastal "
        "residential market; coverage is 100\u202f% but the treated mean "
        "is noisier than for the other three Costcos. Lake Macquarie has "
        "a small number of intermittent monthly gaps (largest 3 months) "
    ...
    story.append(Paragraph(
        ...
        "(postcode &times; month) panel observations. Per-state breakdown "
        "of the 362 / 446 / 0 drops is in Section 2(g).",
        body_style,
    ))
    story.append(Paragraph(
...
def main():
    # Build condensed Section 1 in memory
    print("Building condensed Section 1...")
    sec1_bytes = build_section_1_bytes()

    # Combine with existing Sections 2 and 3
    writer = PdfWriter()

    sec1_reader = PdfReader(io.BytesIO(sec1_bytes))
    for page in sec1_reader.pages:
        writer.add_page(page)
    print(f"  Section 1 (condensed): {len(sec1_reader.pages)} pages")

    for section_file in ["plan_of_attack_section_2.pdf",
                         "plan_of_attack_section_3.pdf"]:
        path = DELIVERABLES / section_file
        if not path.exists():
            raise FileNotFoundError(f"Missing: {path}")
        reader = PdfReader(str(path))
        for page in reader.pages:
            writer.add_page(page)
        print(f"  {section_file}: {len(reader.pages)} pages")

    with open(OUTPUT, "wb") as f:
        writer.write(f)
    print(f"\nWrote {OUTPUT} ({len(writer.pages)} pages total)")
    print(f"Original combined PDF is untouched.")
